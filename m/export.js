...
var check_local_storage=function($a,N){
	...
	if($vm.module_list[href]!=undefined){
		...
		if(tb!=undefined){
			...
			if(localStorage.getItem("export."+tb+".storageTime")==undefined){
				//No record in memory)
				//Export records
				setTimeout(function(){move_to_local_storage($a,true)}, N*1000);
			}
			else{
				...
				setTimeout(function(){
					...
					if(store_time>sub_time){
						if(localStorage.getItem("export."+tb+".data")==undefined){
							//Export records
							move_to_local_storage($a,true)
						}
						else{
							//Read from memory
							move_to_local_storage($a,false)
						}
					}
					else{
						//Export records
...
//--------------------------------------------------------			
var move_to_local_storage=function($a,exp){
	var href=$a.attr('href');
	href=href.replace('?\/','');
	if($vm.module_list[href]!=undefined){
		var tb=$vm.module_list[href].Table;
...
var combine_output=function(output,participant){
	...
	var all="";
	var final=[];
	//Combine all reords for ENROLLED participant
